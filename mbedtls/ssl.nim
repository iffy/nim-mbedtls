...
{.compile: "./mbedtls/csources/library/ssl_client.c".}
{.compile: "./mbedtls/csources/library/ssl_tls12_client.c".}
...
template mb_ssl_conf_own_cert*(conf: mbedtls_ssl_config,
  ...
  if ret != 0:
    raise newException(MbedTLSError, $(ret.mbedtls_high_level_strerr()))
template mb_ssl_conf_dtls_cookies*(conf: mbedtls_ssl_config, cookie: pointer) =
...
template mb_ssl_set_client_transport_id*(ssl: mbedtls_ssl_context, ipv4: array[4, uint8]) =
  let ret = mbedtls_ssl_set_client_transport_id(addr ssl, addr ipv4[0], 4)
  if ret != 0:
    raise newException(MbedTLSError, $(ret.mbedtls_high_level_strerr()))
template mb_ssl_set_client_transport_id*(ssl: mbedtls_ssl_context, ipv6: array[16, uint8]) =
